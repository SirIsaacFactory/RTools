...
main <- function(infile, procname) {
  ...
  resultcsv     <- paste(infilebase,"_", procname, "_WorkingSet2.csv", sep="")
  resultpdf     <- paste(infilebase,"_", procname, "_WorkingSet2.pdf", sep="")
  ...
  #-----------------------------------------------------------------------------
  if(length(df) < 2) {
    print(paste("There must be two ore more", procname, "process Working Set"))
    return(error_end)
  }
